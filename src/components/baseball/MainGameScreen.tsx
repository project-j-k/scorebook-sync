...
import { GameState, GameAction, RunnerOutInfo, RunnerAdvanceInfo } from '@/types/baseball';
...
import { RunnerControlModal } from './RunnerControlModal';
import { Undo2, ClipboardList, Square } from 'lucide-react';
...
export const MainGameScreen = ({ state, dispatch, onRecordPlay, syncStatus, shareCode, onGenerateShareCode, isViewOnly = false, isStandalone = false }: MainGameScreenProps) => {
  ...
  const [showRunnerControl, setShowRunnerControl] = useState(false);
  const [isGeneratingCode] = useState(false);
  ...
  const handleRunnerControlConfirm = (runnersOut: RunnerOutInfo[], runnersAdvance: RunnerAdvanceInfo[], playType: string) => {
    recordPlay('runnerControl', playType, runnersOut, runnersAdvance);
    setShowRunnerControl(false);
  };
  ...
  return (
    <div className="min-h-screen text-foreground pb-28" style={{ background: `linear-gradient(180deg, ${teamColor.primary}15 0%, hsl(var(--background)) 25%)` }}>
      ...
      {!isViewOnly && (
        <div className="px-4 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <LongPressButton 
              label="走塁" 
              onClick={() => hasRunners ? setShowRunnerControl(true) : undefined} 
              className="bg-orange-500 text-white" 
              subOptions={[
                { label: '盗塁成功', onClick: () => recordPlay('steal') },
                { label: '盗塁死', onClick: () => recordPlay('caughtStealing') },
                ...(hasRunners ? [{ label: 'ランナー状況...', onClick: () => setShowRunnerControl(true) }] : []),
              ]} 
            />
          </div>
        </div>
      ...
      {showRunnerControl && <RunnerControlModal runners={state.runners} onConfirm={handleRunnerControlConfirm} onClose={() => setShowRunnerControl(false)} />}
